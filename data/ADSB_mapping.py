...
def calculate_average_heading(df, mode='arrival', n=5):
    if mode == 'arrival':
        headings = df[['x', 'y']].diff().apply(lambda row: np.arctan2(row['x'], row['y']), axis=1)
        n_headings = headings[-n:]
    elif mode == 'departure':
        headings = df[['x', 'y']].diff().apply(lambda row: np.arctan2(row['x'], row['y']), axis=1)
        n_headings = headings[:n]

    average_heading = np.degrees(np.mean(n_headings))
...
def get_data(data_size, multiplier, db_url, id_tab, ADSB_tab, too_short, too_long, arrival_flag, alt_col):
    ...
    query_ids = f"SELECT DISTINCT id FROM {id_tab} WHERE north_bearing is null AND ori_length>={too_short} AND ori_length<={too_long} AND arrival={arrival_flag} ORDER BY RAND() LIMIT {required_data}"
...
req_mul_arr, req_mul_dep = 6.4032, 0.3936
data_size = 10000
    ...
    for id, flt_df in tqdm(ADSB, total=len(ADSB), desc="Processing ADSB", ncols=100):
        ...
        if check_eligible_cupy(flt_df, min_alt_change, FAF, icn_lat, icn_lon, app_sec_rad, alt_col=alt_col, max_range=max_cutoff_range):
            try:
                ...
                num_reject += 1
                db_connection.execute("UPDATE %s SET north_bearing=%f, heading=%f WHERE id=%d" % ('flight', -999, -999, id))
                continue
        else:
            num_reject += 1
            db_connection.execute("UPDATE %s SET north_bearing=%f, heading=%f WHERE id=%d" % ('flight', -999, -999, id))
        ...
        mode = 'arrival' if arrival else 'departure'
        heading = (calculate_average_heading(flt_df, mode='arrival', n=3) + 360) % 360

        df_ls.append(joined_df.dropna()) # append the dataframe # append the bearing
        db_connection.execute("UPDATE %s SET north_bearing=%f, heading=%f WHERE id=%d" % ('flight', north_bearing, heading, id))
